...
void Mech_Drive::Set_Drive(double left_x, double left_y, double right_x, double right_y){
  ...
  // pros::lcd::set_text(2, "right front distance - " + to_string(rfDistance));
  masterDistance += (abs(_right_front_motor_controller->Get_Speed()) + abs(_left_back_motor_controller->Get_Speed()) + abs(_right_back_motor_controller->Get_Speed()) + abs(_left_front_motor_controller->Get_Speed()))/80;
...
void Mech_Drive::Set_Turn(double speed, double direction, double distance)
{
  ...
  void Mech_Drive:: Set_Point_Drive(double speed, double direction, double distance)
  {
    ...
    if (std::abs(actualDistance - distance) > distance/100 + 15 || this->Get_Speed() > 20)
    {
      ...
      // Set_Drive(0, 0, 0, 0);
      Stop();
      pros::lcd::set_text(4, "HELLO THERE");
     _is_running = false;
}
