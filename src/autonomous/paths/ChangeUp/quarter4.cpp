...
AutoSequence *Auton::CUS_Q4 = AutoSequence::FromTasks({
    SingleRun([](void) -> void { position_tracker->Set_Position({-53, 0}, -M_PI); }),
    ...
    AutoPath({-55.5, 55.5}, 3 * M_PI / 4, 127),
    ...
    AutoCurve({-50, 53}, M_PI / 4 + 0.15, {-56, 48}, 0, 127, 3).AddRun([](void) -> void {
    ...
    //change
    AutoCurve({-16, 48}, 0, {0, 57.5}, M_PI / 2, 127, 3).AddRun([](void) -> void {
    ...
    AutoTask::AutoDelay(3000).AddRun([](void) -> void {
        intake->Set_Intake(127);
        indexer->Set_Indexer(127, true);
    }),
